...
class SortedTableMap(MapBase):
    ...
    def __getitem__(self,k):
        """Return value associated with key k (else KeyError)"""
    ...
    def __delitem__(self,k):
        """Remove item associated with key k (else raise error if not found)"""
        j = self._find_index(k,0,len(self._table)-1)
        if j == len(self._table) or self._table[j]._key !=k:
            raise KeyError("Key Error : "+repr(k)) 
        self._table.pop(j)
    ...
    def find_min(self):
        """Return (key,value) tuple with minimum key or None if empty"""
    ...
    def find_max(self):
        """Return (key,value) tuple with maximum key or None if empty"""
        if len(self._table)>0:
